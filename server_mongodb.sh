...
run() {
    ...
    if [ "$storage" = "mongodb" ];then
        ...
        if [ ${load_count} != 6710886 -a ${load_count} != 85894846 ];then
          # scan only
        ...
        if [ ${load_count} == 85894846 -o ${load_count} == 6710886 -o ${load_count} == 131072 ];then
            # scanvalue only with index，256GB
            echo "SCANVALUE $storage ($recordName) with index..."
            ./bin/go-ycsb run ${storage} -P workloads/workload_SCANVALUE > logs/${storage}_${recordName}_SV_WITH_INDEX.txt -p fieldlength=${fieldLength} -p fieldcount=${fieldCount} -p operationcount=${scanCount} -p recordcount=${OPERATIONCOUNT} -p hasIndex=true -p dropIndex=true -p dropDatabase=true -p mongodb.indexs=field0,field1,field2,field3,field4
        else
            # scanvalue only with index
            echo "SCANVALUE $storage ($recordName) with index..."
            ./bin/go-ycsb run ${storage} -P workloads/workload_SCANVALUE > logs/${storage}_${recordName}_SV_WITH_INDEX.txt -p fieldlength=${fieldLength} -p fieldcount=${fieldCount} -p operationcount=${scanCount} -p recordcount=${OPERATIONCOUNT} -p hasIndex=true -p dropIndex=true -p dropDatabase=false -p mongodb.indexs=field0,field1,field2,field3,field4
...
echo start server_mongo.sh ... && date
...
cd ${TEST_TOOL_PATH}
run mongodb 1M_256G 209716 $FIELDCOUNT 5 1024 131072 131072
...
cd ${TEST_TOOL_PATH}
run mongodb 20kb_256G 4096 $FIELDCOUNT 5 1024 6710886 6710887
...
cd ${TEST_TOOL_PATH}
run mongodb 200b_256G 320 $FIELDCOUNT 5 1024 85894846 85894846
